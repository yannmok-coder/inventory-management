...
// Supabase Auth는 이메일 기반 로그인만 지원하므로, 로그인용 "아이디"를 내부적으로만
...
async function loadState(mgmtUnit) {
  ...
  const secretsById = {};
  (adminSecrets.data || []).forEach((r) => { secretsById[r.id] = { militaryId: r.military_id, birthDate: r.birth_date }; });

  return {
    admins: (admins.data || []).map(mapAdminFromDb).map((a) => ({ ...a, militaryId: secretsById[a.id]?.militaryId || '', birthDate: secretsById[a.id]?.birthDate || '' })),
...
function LoginGate({ onLogin, onSignup, onRecover }) {
  ...
  const submit = async () => {
    ...
    // recover
    if (!username.trim() || !militaryId.trim() || !birthDate) { setError('아이디, 군번, 생년월일을 모두 입력해주세요.'); return; }
    if (!password) { setError('새 비밀번호를 입력해주세요.'); return; }
    if (password !== confirm) { setError('새 비밀번호가 서로 일치하지 않습니다.'); return; }
    setSubmitting(true);
    const result = await onRecover(username, militaryId, birthDate, password);
    setSubmitting(false);
      ...
      <div className="jamul-card w-full max-w-md p-8">
        ...
        {mode === 'recover' && (
          <div className="mt-6 mb-5">
            ...
            <p className="text-xs mt-1" style={{ color: 'var(--ink-soft)' }}>가입 시 등록한 아이디·군번·생년월일로 본인 확인 후 새 비밀번호를 설정합니다.</p>
          </div>
        )}
        ...
        <div className="space-y-3">
          {mode === 'signup' && (
            <p className="text-xs px-3 py-2 rounded-lg" style={{ background: 'var(--warning-bg)', color: 'var(--warning)' }}>
              가입 신청 후 관리자의 승인이 있어야 로그인할 수 있습니다.
            </p>
          )}
          <div>
 ...
 * 관리자 관리
 * ------------------------------------------------------------- */
function AdminEditModal({ target, adminActions, onClose }) {
  ...
  return (
    <Modal title={`계정 정보 수정 · ${target.name}`} onClose={onClose}>
      <div className="space-y-3">
        <div>
          <label className="text-xs font-medium" style={{ color: 'var(--ink-soft)' }}>아이디</label>
          <div className="flex gap-2 mt-1">
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            <label className="text-xs font-medium" style={{ color: 'var(--ink-soft)' }}>군번</label>
            <input value={militaryId} onChange={(e) => setMilitaryId(e.target.value)} className="w-full mt-1 border rounded-lg px-3 py-2 text-sm jamul-focus" style={{ borderColor: 'var(--border)' }} />
          </div>
          <div className="flex-1">
            <label className="text-xs font-medium" style={{ color: 'var(--ink-soft)' }}>생년월일</label>
            <input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} className="w-full mt-1 border rounded-lg px-3 py-2 text-sm jamul-focus" style={{ borderColor: 'var(--border)' }} />
          </div>
        </div>
        <button onClick={submit} disabled={submitting} className="jamul-btn-primary w-full rounded-lg py-2.5 text-sm font-medium mt-2">
          {submitting ? '저장 중...' : '저장'}
        </button>
      </div>
    </Modal>
    ...
    <div className="space-y-4">
      {pending.length > 0 && (
        <div className="jamul-card overflow-x-auto">
          <div className="px-4 py-3 border-b" style={{ borderColor: 'var(--border)' }}>
            <h3 className="text-sm font-bold flex items-center gap-1.5" style={{ color: 'var(--ink)' }}>
              <AlertTriangle size={14} color="var(--warning)" />가입 승인 대기 ({pending.length})
            </h3>
          </div>
          <table className="w-full text-sm">
            <thead>
            ...
            <tbody>
              {pending.map((a) => (
                <tr key={a.id} className="border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
                  <td className="px-4 py-3" style={{ color: 'var(--ink-soft)' }}>{a.username || '-'}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    <button
                      onClick={() => setConfirmState({
                        message: `'${a.name}'님의 가입 신청을 거절하시겠습니까?`,
      ...
      <div className="jamul-card overflow-x-auto">
        ...
        <p className="text-xs px-4 py-3" style={{ color: 'var(--ink-soft)' }}>
          새 사용자는 로그인 화면의 <b>신규 등록</b> 탭에서 아이디와 비밀번호를 정해 직접 가입 신청합니다. 이곳에서 승인해야 로그인할 수 있습니다.
        </p>
      </div>

      {editTarget && <AdminEditModal target={editTarget} adminActions={adminActions} onClose={() => setEditTarget(null)} />}
    </div>
  );
}
    ...
    <div className="space-y-4">
      <div className="jamul-card p-4 max-w-md">
        ...
        <div className="space-y-3">
          ...
          {profileError && (
            <p className="text-xs px-3 py-2 rounded-lg" style={{ background: 'var(--danger-bg)', color: 'var(--danger)' }}>{profileError}</p>
          )}
          {profileNotice && (
            <p className="text-xs px-3 py-2 rounded-lg" style={{ background: 'var(--success-bg)', color: 'var(--success)' }}>{profileNotice}</p>
          )}
          <button onClick={saveProfile} disabled={savingProfile} className="jamul-btn-primary w-full rounded-lg py-2.5 text-sm font-medium">
            {savingProfile ? '저장 중...' : '정보 저장'}
          </button>
          <p className="text-xs" style={{ color: 'var(--ink-soft)' }}>아이디를 변경하면 다음 로그인부터 새 아이디를 사용해야 합니다.</p>
        </div>
      </div>
...
export default function App() {
  ...
  const loginAs = async (username, password) => {
    ...
    const { data, error } = await supabase.auth.signInWithPassword({ email: usernameToEmail(trimmed), password });
    if (error) return { ok: false, message: '아이디 또는 비밀번호가 올바르지 않습니다.' };
    ...
    if (profile.status === 'pending') {
      await supabase.auth.signOut();
      return { ok: false, pending: true, message: '아직 승인되지 않은 계정입니다.' };
    }
    return { ok: true };
  };
  ...
  const signUpAdmin = async (username, name, password, militaryId, birthDate, mgmtUnit) => {
    ...
    if (error) {
      ...
      return { ok: false, message: already ? '이미 등록된 아이디입니다. 로그인해주세요.' : `등록에 실패했습니다: ${error.message}` };
    }
    ...
    if (status === 'pending') {
      await supabase.auth.signOut();
      return { ok: false, pending: true, message: '관리자의 승인 후 사용할 수 있습니다.' };
    }
    return { ok: true };
  };

  const recoverPassword = async (username, militaryId, birthDate, newPassword) => {
    const trimmed = (username || '').trim();
    const trimmedMilId = (militaryId || '').trim();
  ...
  const updateOwnProfile = async ({ username, name, militaryId, birthDate }) => {
    const trimmedUsername = (username || '').trim();
    const trimmedName = (name || '').trim();
    const trimmedMilId = (militaryId || '').trim();
    ...
    const { data, error } = await supabase.rpc('update_own_profile', {
      ...
      p_new_military_id: trimmedMilId,
      p_new_birth_date: birthDate,
    });
    if (error) {
      const dup = /duplicate|already|unique/i.test(error.message || '');
      return { ok: false, message: dup ? '이미 사용중인 아이디입니다.' : `수정에 실패했습니다: ${error.message}` };
    }
  ...
  const adminActions = {
    ...
    adminUpdateAccount: async (id, { username, name, militaryId, birthDate }) => {
      const trimmedUsername = (username || '').trim();
      const trimmedName = (name || '').trim();
      const trimmedMilId = (militaryId || '').trim();
      ...
      if (error) {
        const dup = /duplicate|already|unique/i.test(error.message || '');
        showToast(dup ? '이미 사용중인 아이디입니다.' : `수정에 실패했습니다: ${error.message}`, 'danger');
        return false;
      }
      if (!data) { showToast('수정에 실패했습니다.', 'danger'); return false; }
      setState((prev) => ({
        ...prev,
        admins: prev.admins.map((a) => (a.id === id ? { ...a, name: trimmedName, username: trimmedUsername, militaryId: trimmedMilId, birthDate } : a)),
      }));
    ...
    <div className="jamul-root flex h-screen w-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto jamul-scrollbar p-6">
          ...
          {safeActiveTab === 'issuance' && <IssuanceView state={state} calc={calc} actions={actions} showToast={showToast} />}
          {safeActiveTab === 'persons' && <PersonsView state={state} calc={calc} actions={actions} showToast={showToast} setConfirmState={setConfirmState} />}
          {safeActiveTab === 'itemsManage' && <ItemsView state={state} calc={calc} actions={actions} setConfirmState={setConfirmState} />}
          {safeActiveTab === 'maintenance' && <MaintenanceView state={state} calc={calc} actions={actions} setConfirmState={setConfirmState} />}
